...
const getRightBorderColorUnique = (props: NodeDisplayProps, color: string) => {
  return getRightBorderColor(props, () => {
    return color;
  });
};
...
const getBottomBorderColorUnique = (props: NodeDisplayProps, color: string) => {
  return getBottomBorderColor(props, () => {
    return color;
  });
};
...
const NodeDisplay = styled.div`
  ...
  &.player-found {
    ...
    border-right: solid 3px
      ${(props) => getRightBorderColorUnique(props, START_COLOR)};
    border-bottom: solid 3px
      ${(props) => getBottomBorderColorUnique(props, START_COLOR)};
  }

  &.found {
    background: ${FOUND_COLOR};
    border-right: solid 3px
      ${(props) => getRightBorderColorUnique(props, FOUND_COLOR)};
    border-bottom: solid 3px
      ${(props) => getBottomBorderColorUnique(props, FOUND_COLOR)};
    transition: background ${getDelay}ms ${getFoundDelay}ms,
      border ${getDelay}ms ${getFoundDelay}ms;
  }

  &.path {
    background: ${PATH_COLOR};
    border-right: solid 3px
      ${(props) => getRightBorderColorUnique(props, PATH_COLOR)};
    border-bottom: solid 3px
      ${(props) => getBottomBorderColorUnique(props, PATH_COLOR)};
    transition: background ${getDelay}ms ${getPathDelay}ms,
      border ${getDelay}ms ${getPathDelay}ms;
  }
`;
